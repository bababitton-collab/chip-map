...
(function(){
  ...
  const LOCK_DUMMY = 'Locked question — the full text arrives in the weekly mail, with what to listen for on the call';
  ...
  document.getElementById('wlist').innerHTML = rows.map(w=>{ const dd=days(w.d); return `<div class="wrow ${dd<=21?'soon':''}">
    ...
    <div class="wq">${w.locked? `<span class="blur">${LOCK_DUMMY}</span><span class="lockline">🔒 The full question — <b>in the weekly mail</b></span>` : `${w.q}<span class="lis"><b>Listen for:</b> ${w.listen}</span>`}</div></div>`; }).join('');
